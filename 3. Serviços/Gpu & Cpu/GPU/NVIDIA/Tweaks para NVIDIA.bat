...
echo -[MEM] Forcando memoria contigua (menos fragmentacao)
...
echo -[OK] Memoria contigua aplicada
echo.

echo -[HDCP] Desligando HDCP (sem travas extras)
chcp 437 >nul 2>&1
...
echo -[OK] HDCP tweak aplicado
echo.

echo -[GPU] Desativando preemption (foco em latencia)
...
echo -[OK] Preemption tweak aplicado
echo.

echo -[HAGS] Ativando aceleracao por hardware (HwSchMode)
reg add "HKLM\SYSTEM\CurrentControlSet\Control\GraphicsDrivers" /v "HwSchMode" /t REG_DWORD /d 2 /f

echo -[OK] HAGS aplicado
echo.

echo -[LAT] Ajustando tolerancia de latencia NVIDIA e sistema
...
echo -[TEL] Desativando telemetria da NVIDIA %g%(se ja estiver off pode dar erro)
...
echo -[CLEAN] Apagando arquivos de telemetria (se existir)
